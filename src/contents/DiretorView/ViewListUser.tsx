...
const ViewListUser = (props: propsView) => {
  ...
  const handleUsuarios = async () => {
    ...
    if (dataUser) {
      ...
      try {
        const response = await api.get('/usuario', { params: { all: true , attribute : "escola_uuid", value: tempUser.user.escola_uuid } })
        const users: dataUser[] = response.data
        if (response) {
          setUsuarios(users.filter(item => item.access_level <= 4))
        }
      } catch (error) {
        console.log(error)
      }
